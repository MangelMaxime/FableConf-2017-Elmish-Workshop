...
module morgan =
    ...
    and [<AllowNullLiteral>] Morgan =
        [<Emit("$0($1...)")>] abstract Invoke: format: string * ?options: Options -> express.RequestHandler
        [<Emit("$0($1...)")>] abstract Invoke_combined: ?options: Options -> express.RequestHandler
        [<Emit("$0($1...)")>] abstract Invoke_common: ?options: Options -> express.RequestHandler
        [<Emit("$0($1...)")>] abstract Invoke_dev: ?options: Options -> express.RequestHandler
        [<Emit("$0($1...)")>] abstract Invoke_short: ?options: Options -> express.RequestHandler
        [<Emit("$0($1...)")>] abstract Invoke_tiny: ?options: Options -> express.RequestHandler
